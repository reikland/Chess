...
from chess_engine.ai.engine import choose_move
...
class ChessApp:
    def __init__(self, root: tk.Tk) -> None:
        ...
        self.ai_depth = tk.IntVar(value=2)
    ...
    def _build_ui(self) -> None:
        ...
        mode_frame.grid(row=0, column=2, padx=10)

        ttk.Checkbutton(mode_frame, text="Activer l'IA", variable=self.ai_enabled, command=self._on_ai_toggle).grid(
            row=0, column=0, padx=5
        )
        ttk.Label(mode_frame, text="Couleur IA:").grid(row=0, column=1, sticky="e")
        ttk.OptionMenu(mode_frame, self.ai_color, self.ai_color.get(), "white", "black", command=self._on_ai_color).grid(
            row=0, column=2, padx=5
        )
        ttk.Label(mode_frame, text="Profondeur:").grid(row=0, column=3, sticky="e")
        ttk.Spinbox(mode_frame, from_=1, to=4, textvariable=self.ai_depth, width=3).grid(row=0, column=4)
    ...
    def reset_game(self) -> None:
        ...
        self.draw_board()
        self.root.after(10, self._maybe_play_ai)
    ...
    def _on_ai_color(self, *_: object) -> None:
        self._maybe_play_ai()
    ...
    def _maybe_play_ai(self) -> None:
        ...
        if self.game.game_status() != "ongoing":
            ...
            self.draw_board()
            return
        if self.game.turn != self.ai_color.get():
            return

        depth = max(1, int(self.ai_depth.get() or 1))
        move = choose_move(self.game.board, depth, self.game.turn)
        if move is None:
            self.status_var.set(self._status_text())
            self.draw_board()
            return
        ...
        # Apply chosen move
        self.game.board.apply_move(move)
        ...
        # If the AI plays both sides (for quick analysis), keep running until human's turn
        if self.game.turn == self.ai_color.get() and self.game.game_status() == "ongoing":
            self.root.after(10, self._maybe_play_ai)
